...
export default function ResultColumn({ title, items, onAdd, onSelectSchool, onFavorite }) {
  ...
  return (
    <section className="result-column">
      ...
      {visibleItems.map((item) => (
        <ResultCard key={item.id} item={item} onAdd={onAdd} onSelectSchool={onSelectSchool} onFavorite={onFavorite} />
      ))}
    </section>
  )
}
